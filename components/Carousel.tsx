  ...
  const slideElements = carousel.products.map((product, index) => (
    <div key={index}>
      <Link className="w-full" href={`/produtos/${product.slug.current}`}>
        <div
          className={` rounded-xl text-color-5 shadow-lg hover:border-x bg-color-3  border-color-5/10 relative px-6 py-6 cursor-pointer transition-all hover:shadow-xl w-[235px] md:w-[240px] flex items-center justify-center flex-col text-center`}
